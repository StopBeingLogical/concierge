...
CANDIDATES = [
    ...
    {"tag": "phi4:14b",           "family": "Phi",     "tier": 1, "size_b": 14},
    # Tier 2 — if time permits
    {"tag": "mistral-small:22b",  "family": "Mistral", "tier": 2, "size_b": 22},
    {"tag": "gemma3:12b",         "family": "Gemma",   "tier": 2, "size_b": 12},
    {"tag": "gemma3:27b",         "family": "Gemma",   "tier": 2, "size_b": 27},
...
TESTS = {
    ...
    1: {
        ...
        "prompt": (
            "I am refactoring a distributed job scheduler. Workers currently heartbeat "
            "to a central Redis instance, but we're seeing connection exhaustion at 500+ nodes. "
            "I'm considering moving to a gossip-based health check. "
            "Do not provide a solution. Ask me three probing questions about my infrastructure "
            "constraints that I might have overlooked."
        ),
        "tool_schema": None,
        "scoring_guide": (
            "5 — Identifies split-brain risk, broadcast storm overhead, or quorum failure modes. "
            "Stays in investigative mode, no solution offered.\n"
            "4 — Good questions, misses one key angle.\n"
            "3 — Reasonable questions but generic, no architectural insight.\n"
            "2 — Provides a solution despite instruction. Breaks the constraint.\n"
            "1 — Off-topic, incoherent, or refuses."
        ),
    },
...
def run_test_1(model: str) -> dict:
    """Rubber Duck — conversational reasoning."""
    t = TESTS[1]
    ...
    if thinking_leak:
        print_warn("Thinking token leak detected in response.")

    print(f"\n  Response preview:\n  {response[:400].replace(chr(10), chr(10)+'  ')}\n  ...")
